...
function hasResvg() {
    return !!loadOptionalModule('@resvg/resvg-js');
}
...
function renderSvgWithResvg(inputPath, outputPath, size) {
    const mod = loadOptionalModule('@resvg/resvg-js');
    if (!mod) {
        return false;
    }
...
async function convertToPng(inputPath, outputPath, size) {
    ...
    if (inputFormat === 'svg' && hasResvg()) {
        try {
            if (renderSvgWithResvg(inputPath, outputPath, size)) {
                return true;
            }
        } catch (e) {
            console.warn(`[icon-converter] resvg failed: ${e.message}`);
        }
    }
